...
from stgcn_traffic_prediction.dataloader.STMatrix5 import STMatrix
...
def _loader(f, nb_flow, traffic_type):
    if nb_flow == 1:
        ...
        result = data.transpose((0,3,1,2)).reshape((-1,1,height*width))
        return result
...
def load_data(path, traffic_type, closeness_size, period_size, trend_size, len_test, nb_flow):
    ...
    for data, index in zip(data_all_mmn, index_all):
        ...
        st = STMatrix(data, index, 33)
        _xc, _xp, _xt, _y, _timestamps_y = st.create_dataset(
            len_closeness=closeness_size, len_period=period_size, len_trend=trend_size,PeriodInterval=11)
