...
class close(nn.Module):
    def __init__(self,k,N,model_d, temporal_arch: str = 'transformer'):
        super(close,self).__init__()
        # 保持默认 transformer 行为
        self.c_temporal = make_temporal_model(temporal_arch, k+1, 1, N, model_d)
        self.k = k
    ...
    def forward(self,x_c,x_p,tgt_mode,mode,flow,adj=None,index=None,x_t=None):
        ...
        tx_c = torch.cat([sx_c[:,flow].unsqueeze(-1),selected.transpose(-1,-2)],dim=-1).to(sx_c.device)
        #(bs,N,c,k+1)
        ...
        tgt_mask_c = c_subsequent_mask(len_closeness).to(sx_c.device)
        if(tgt_mode=='c'):
            tgt_c = sx_c[:,flow].unsqueeze(-1).cuda()
        elif(tgt_mode=='r'):
            tgt_c = torch.rand((bs,N,len_closeness,1)).cuda()
        elif(tgt_mode=='p'):
            tgt_c = torch.mean(x_p[:,:,:,flow],dim=1).transpose(1,2).unsqueeze(-1).to(x_p.device)
        elif(tgt_mode=='t'):
