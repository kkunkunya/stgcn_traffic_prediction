...
from .utils2 import getadj,getA_cosin,getA_corr
...
class T_STGCN(nn.Module):
    def __init__(self,len_closeness, external_size, N, k, spatial, s_model_d,c_model_d,p_model_d,t_model_d,dim_hid=16, drop_rate=0.1, temporal_arch: str = 'transformer'):
        ...
        self.c_temporal = close(k,N,c_model_d, temporal_arch=temporal_arch)
        self.p_temporal = period(len_closeness,N,p_model_d, temporal_arch=temporal_arch)
    ...
    def forward(self,x_c,mode,c,s,FS,c_tgt,s_tgt,flow,x_p,x_t=None):
        ...
        # get adj
        # utils2.getA_* assumes view((bs, N, c*1)), so pass a single-flow tensor to avoid shape mismatch
        x_for_adj = x_c.permute((0, 2, 3, 1))[:, :1, :, :]  # (bs, 1, N, c)
        if(mode=='cos'):
            adj = getA_cosin(x_for_adj)
        elif(mode=='corr'):
            adj = getA_corr(x_for_adj)
        else:
            raise Exception('wrong adj mode')
        index = torch.argsort(adj,dim=-1,descending=True)[:,:,0:self.k]
        if(s):
            # spatial branch expects adj built on single-flow; we already pass x_for_adj above.
