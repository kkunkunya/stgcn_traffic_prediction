...
def train_epoch(data_type,epoch):
    ...
    if (opt.period_size > 0) & (opt.close_size > 0) & (opt.trend_size > 0):
        for idx, (c, p, t, target) in enumerate(data):
            ...
            optimizer.zero_grad()
            model.zero_grad()
            # correct arg order: (x_c, mode, c, s, FS, c_tgt, s_tgt, flow, x_p, x_t)
            pred = model(c.float(),opt.mode,opt.c,opt.s,opt.FS,opt.c_t,opt.s_t,opt.flow,p.float(),t.float())
...
def predict(test_type='train'):
    ...
    # ensure all metrics are plain Python numbers for formatting
...
if __name__ == '__main__':
    # robust absolute path to dataset
    path = os.path.join(PROJECT_ROOT, 'stgcn_traffic_prediction', 'all_data_sliced2.h5')
