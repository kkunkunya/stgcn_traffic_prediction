...
def predict(test_type='train'):
    ...
    for i in range(final_predict.shape[2]):
        plt.figure(figsize=(10, 5))
        plt.plot(final_predict[:, :, i], label='Prediction')
        plt.plot(ground_truth[i:, :, i], label='Ground Truth')
        plt.xlabel('Time')
        plt.ylabel('Value')
...
def train_valid_split(dataloader, test_size=0, shuffle=True, random_seed=0):
    length = len(list(dataloader))#####0.2
...
if __name__ == '__main__':
    ...
    train_idx, valid_idx = train_valid_split(train_data,0)
