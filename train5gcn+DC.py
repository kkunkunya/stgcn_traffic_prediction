...
sys.path.append('../../')
from stgcn_traffic_prediction.dataloader.milano_crop5 import load_data
...
from stgcn_traffic_prediction.utils.parser4gcn import getparse
...
def predict(test_type='train'):
    ...
    # %%%%20250304
    sklearn_nrmse = sklearn_nrmse.item() if isinstance(sklearn_nrmse, torch.Tensor) else sklearn_nrmse
    sklearn_mae = sklearn_mae.item() if isinstance(sklearn_mae, torch.Tensor) else sklearn_mae
    sklearn_r2 = sklearn_r2.item() if isinstance(sklearn_r2, torch.Tensor) else sklearn_r2
    # mrt = mrt.item() if isinstance(mrt, torch.Tensor) else mrt

    mse = mse.item() if isinstance(mse, torch.Tensor) else mse
    rmse = rmse.item() if isinstance(rmse, torch.Tensor) else rmse
    nrmse = nrmse.item() if isinstance(nrmse, torch.Tensor) else nrmse
    ...
    #     sklearn_mse, sklearn_rmse, sklearn_nrmse, sklearn_mae, sklearn_r2,
    #     mrt) + ' [Real MSE]:{:0.5f}, [Real RMSE]:{:0.5f}, [Real NRMSE]: {:0.5f}, [Real MAE]:{:0.5f}, [Real R2]: {:0.5f}'.format(
    #     mse, rmse, nrmse, mae, r2)
